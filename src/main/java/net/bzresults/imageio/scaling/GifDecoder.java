...
public class GifDecoder {
  ...
  public int read(String name) {
    status = STATUS_OK;
    try {
      //name = name.trim().toLowerCase();  // LAW replaced with lines below
      //if ((name.indexOf("file:") >= 0) || (name.indexOf(":/") > 0)) { // LAW replaced with lines below
        String lcName = name.trim().toLowerCase();
        if ((lcName.indexOf("file:") >= 0) || (lcName.indexOf(":/") > 0)) {
